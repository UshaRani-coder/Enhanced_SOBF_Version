...
const Recent_Activities = React.memo(() => {
  ...
  // Sorting the posts based on date in the selected order
  const displayedPosts = useMemo(() => {
    if (!posts || posts?.length === 0) return [];

    // Sort posts by date based on the sortOrder
  ...
  return (
    <div className={`flex flex-col items-center mb-[30px] ${isHomePage ? 'mt-[30px]' : 'mt-[120px]'}`}>
      ...
      <InfiniteScroll
        ...
        loader={<img src={loader} alt="Loading..." className="w-[20px] h-[20px] align-center" />}
            ...
            >
              ...
              <div className="px-[10px]">
                <div className="flex items-center gap-x-[5px] mt-[15px]">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="w-[10px]">
                    <path d="M464 256A208 208 0 1 1 48 256a208 208 0 1 1 416 0zM0 256a256 256 0 1 0 512 0A256 256 0 1 0 0 256zM232 120l0 136c0 8 4 15.5 10.7 20l96 64c11 7.4 25.9 4.4 33.3-6.7s4.4-25.9-6.7-33.3L280 243.2 280 120c0-13.3-10.7-24-24-24s-24 10.7-24 24z" />
                  </svg>
                  <span className="text-[13px]">{formatDate(activity.date)}</span>
                </div>
                <h1 className="font-bold text-xl line-clamp-1">{activity.title}</h1>
                <p className="md:text-lg line-clamp-4">{activity.description}</p>
