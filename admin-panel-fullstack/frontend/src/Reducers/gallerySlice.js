...
export const getGalleryImages = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || "Failed to fetch gallery images");
    }
  }
);
...
export const addGallery = createAsyncThunk('gallery/addGalleryImage', async (postData, { rejectWithValue }) => {
  try {
    const response = await createGallery(postData)
    console.log("response", response);
    return response.data;
  } catch (error) {
    return rejectWithValue(error.response?.data?.message || 'Failed to add gallery image');
  }
});

// Update a gallery image
export const updateGalleryImage = createAsyncThunk('gallery/updateGalleryImages', async ({ id, updatedData }, { rejectWithValue }) => {
  try {
    const response = await updateGallery(id, updatedData);
    return response.data;
  } catch (error) {
    return rejectWithValue(error.response?.data?.message || 'Failed to update gallery image');
  }
});

// Remove a gallery image
export const removeGallery = createAsyncThunk('gallery/removeGalleryImage', async (id, { rejectWithValue }) => {
  try {
    await deleteGallery(id);
    return id;
  } catch (error) {
    return rejectWithValue(error.response?.data?.message || 'Failed to remove gallery image');
  }
});
