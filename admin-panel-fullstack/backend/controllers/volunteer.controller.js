...
const createVolunteer = async (req, res) => {
  try {
    ...
    // Send confirmation email (do not block registration)
    try {
      await sendVolunteerConfirmationEmail(savedVolunteer);
    } catch (emailError) {
      logger.error(
        `Volunteer confirmation email failed for ${savedVolunteer.email}`,
        emailError,
      );
    }
...
const deleteVolunteer = async (req, res) => {
  try {
    const deletedVolunteer = await volunteerModel.findOneAndDelete({
      volunteerId: req.params.id,
    });
    if (!deletedVolunteer) {
      return res.status(404).json({
